using CalculoAplicación.Figuras;
using System;
...
namespace CalculoAplicación
{
    class Grafica : Panel
    {
        ...
        private int aux, avx, ba, bb, bc, bd, be, rectanguloMenor = -1, rectanguloMayor = -1, max, nv, freljord = 40;
        private double pv;
        private bool band, bond = false;
        private string tipo;
        string opcional;
        // Gráficos
        private Pen p;
        ...
        public void CambioColor(string cambio, Color color)
        {
            switch (cambio)
            {
                case "Principal":
                    colorPrimario = new Color();
                    colorPrimario = Color.DodgerBlue;
                    break;
                case "Secundario":
                    colorSecundario = new Color();
        ...
        private void Grafica_Paint(object sender, PaintEventArgs e)
        {
            // Crea los graficos
            g = e.Graphics;
            p = new Pen(new SolidBrush(Color.White));
            pincelPrincipal = new SolidBrush(Color.DodgerBlue);
            pincelSecundario = new SolidBrush(colorSecundario);

            // Dibuja un cilindro
            //puntos
            int cx1 = 120, cy1 = 50;
            int cx3 = 450, cx2 = 450 - cx1;
            int cy3 = 400, cy2 = 400 - cx1;

            g.DrawRectangle(p, new Rectangle(10, 10, 567, 515));
            
            fg3d.Cilindro(cx1, cx2, cx3, cy1, cy2, cy3, g, p);
            ...
            if (bond)
            {
                ...
                {
                    for (int gh = 400; gh > freljord; gh -= 10)
                    {
                        fg3d.CirculoLleno(120, gh, 330, 80, g, pincelPrincipal);
            ...
            fg3d.Cilindro(cx1, cx2, cx3, cy1, cy2, cy3, g, p);
            //fg3d.Cilindro(cx1, cx2, cx3, cy1, cy2, cy3, g, p);
            //fg3d.CilindroLleno(cx1, cx2, cx3, cy1, cy2/2, cy3/2, g, p, pincelPrincipal);
